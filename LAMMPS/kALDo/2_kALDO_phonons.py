...
#	atoms = read(folder_string+'replicated_atoms.xyz',format='xyz')
    """
    Tengo que mirar que es lo que lee kaldo.forceconstants; atoms (probablemente con ase read basta), folder (aquí se pone la dirección donde estan los datos)
    Supongo que si los datos estan en el directorio de trabajo folder puede omitirse???
    """
	forceconstants = ForceConstants(atoms=atoms,
#    folder=folder_string+'/ald')
    folder="trabajo/data" #supongo qui que los datos estan en el directorio trabajo/data
