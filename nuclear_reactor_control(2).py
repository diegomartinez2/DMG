...
# Global lists to store voltage data for plotting
voltage_data = []
time_data = []
start_time = None  # Global variable for start time
...
def draw_plot_window(win, voltage_data, time_data):
    ...
    if len(voltage_data) >= plot_width:
        ...
        time_data.clear()
        global start_time
        start_time = None  # Reset start_time when clearing plot
    ...
    while True:
        ...
        if key == curses.KEY_UP:
            ...
            if selected_button == 0:
                ...
                time_data.clear()
                global start_time
                start_time = None  # Reset start_time on emergency shutdown
        elif key == ord('q'):
            break
