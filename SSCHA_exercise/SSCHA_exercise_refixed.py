...
class Calculo_inicial(object):
    def __init__(self,fichero_ForceFields,fichero_dyn,nqirr,configuraciones,sobol,sobol_scatter,sobol_scramble):
...
#                          max_pop = 200)
        ...
        self.relax.relax(sobol = self.sobol, sobol_scramble=self.sobol_scramble, sobol_scatter = self.sobol_scatter)
...
class Hessiano_Vs_Temperatura(object):
    def __init__(self,T0,temperatura_i,fichero_ForceFields,configuraciones,sobol,sobol_scatter,sobol_scramble):
        # Load the dynamical matrix for the force field
        self.ff_dyn = CC.Phonons.Phonons(fichero_ForceFields, 3)
...
#            self.relax.relax(sobol = False)
            ...
            self.ensemble.generate(self.configuraciones, sobol = self.sobol, sobol_scramble=self.sobol_scramble,sobol_scatter = self.sobol_scatter)
...
class Hessiano_Vs_Configurations(object):
    def __init__(self,T0,fichero_ForceFields,configuraciones,sobol,sobol_scatter,sobol_scramble):
        # Load the dynamical matrix for the force field
        self.ff_dyn = CC.Phonons.Phonons(fichero_ForceFields, 3)
...
#            self.minim.enforce_sum_rule = True  # Lorenzo's solution to the error
    ...
    def dibuja(self):
        ...
        # hessian_data0 = np.loadtxt("0_hessian_vs_configuraciones.dat")
...
#    Temperatura_i = np.linspace(50, 300, 6)
    Temperatura_i = np.linspace(50, 300, 30)
#    Configuraciones_i = [2**4,2**5,2**6,2**7,2**8,2**9,2**10,2**11,2**12,2**13,2**14]
#    Configuraciones_i = [2**9,2**10]
    Configuraciones_i = np.linspace(2**4,2**10,60,dtype=int)#range(16,1024,10)#range(1024,32,-32) #range(32, 512, 2)
...
#    Fichero_dyn_SnTe = "final_sscha_T300_"
    ...
    Calculo = Calculo_inicial(Fichero_ForceFields,Fichero_dyn_SnTe,nqirr,configuraciones,sobol,sobol_scatter,sobol_scramble)
    Calculo.ensambla(T0)
...
#    HessianoVsTemperatura = Hessiano_Vs_Temperatura(T0,Temperatura_i,Fichero_ForceFields,configuraciones,sobol,sobol_scatter,sobol_scramble)
#    HessianoVsTemperatura.ciclo_T(Fichero_final_matriz_dinamica,nqirr)
#    HessianoVsTemperatura.dibuja()

    HessianoVsConfiguraciones = Hessiano_Vs_Configurations(T0,Fichero_ForceFields,Configuraciones_i,sobol,sobol_scatter,sobol_scramble)
#    print ('Configuraciones=',Configuraciones_i)
#    indice=0
