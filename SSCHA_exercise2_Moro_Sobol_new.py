...
class Hessiano_Vs_Temperatura(object):
    def __init__(self,T0,temperatura_i,fichero_ForceFields,sobol,sobol_scatter):
        # Load the dynamical matrix for the force field
        self.ff_dyn = CC.Phonons.Phonons(fichero_ForceFields, 3)
...
#        self.lowest_sscha_mode = []
    ...
    def ciclo_T(self,Fichero_final_matriz_dinamica,nqirr,indice,configuraciones):
        lowest_hessian_mode = []
        lowest_sscha_mode = []
...
#            self.relax = sscha.Relax.SSCHA(self.minim, ase_calculator = self.ff_calculator, N_configs=1000, max_pop=50)
            self.relax = sscha.Relax.SSCHA(self.minim, ase_calculator = self.ff_calculator, N_configs=configuraciones, max_pop=20)
...
#            self.relax.relax(sobol = False)
            ...
            self.ensemble.generate(configuraciones, sobol = self.sobol, sobol_scatter = self.sobol_scatter)
...
#            self.ensemble.generate(5000,sobol = True)
        ...
        np.savetxt("{}_hessian_vs_temperature.dat".format(indice), freq_data, header = "T [K]; SSCHA mode [cm-1]; Free energy hessian [cm-1]")

    def dibuja(self):
        hessian_data = np.loadtxt("0_hessian_vs_temperature.dat")
        hessian_data1 = np.loadtxt("1_hessian_vs_temperature.dat")
        hessian_data2 = np.loadtxt("2_hessian_vs_temperature.dat")
        hessian_data3 = np.loadtxt("3_hessian_vs_temperature.dat")
        hessian_data4 = np.loadtxt("4_hessian_vs_temperature.dat")
#        hessian_data5 = np.loadtxt("5_hessian_vs_temperature.dat")

        plt.figure(dpi = 120)
#        plt.plot(hessian_data[:,0], hessian_data[:,1], label = "Min SCHA freq", marker = ">")
        plt.plot(hessian_data[:,0], hessian_data[:,2], label = "Free energy curvature 2**6", marker = "o")
        plt.plot(hessian_data1[:,0], hessian_data1[:,2], label = "Free energy curvature 2**8", marker = ">")
        plt.plot(hessian_data2[:,0], hessian_data2[:,2], label = "Free energy curvature 2**10", marker = "+")
        plt.plot(hessian_data3[:,0], hessian_data3[:,2], label = "Free energy curvature 2**12", marker = "o")
        plt.plot(hessian_data4[:,0], hessian_data4[:,2], label = "Free energy curvature 2**14", marker = ">")
        plt.axhline(0, 0, 1, color = "k", ls = "dotted") # Draw the zero
        plt.xlabel("Temperature [K]")
        plt.ylabel("Frequency [cm-1]")
        plt.legend()
        plt.tight_layout()
        plt.savefig('Temp_Freq_FEC.png')
        plt.show()

        a = [hessian_data[:,2],hessian_data1[:,2],hessian_data2[:,2],hessian_data3[:,2],hessian_data4[:,2]]
        media_data = sum(a)/5
        plt.plot(hessian_data2[:,0], media_data)
        max_a = np.zeros(len(hessian_data[:,0]))
        min_a = np.zeros(len(hessian_data[:,0]))
        for i in range(len(hessian_data[:,0])):
            max_a[i]=max((a[0][i],a[1][i],a[2][i],a[3][i],a[4][i]))
            min_a[i]=min((a[0][i],a[1][i],a[2][i],a[3][i],a[4][i]))
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('Temp_Freq_FEC2.png')
        plt.show()

        plt.plot(hessian_data[:,0], hessian_data[:,1], label = "Min SCHA freq 2**6", marker = "o")
        plt.plot(hessian_data1[:,0], hessian_data1[:,1], label = "Min SCHA freq 2**8", marker = ">")
        plt.plot(hessian_data2[:,0], hessian_data2[:,1], label = "Min SCHA freq 2**10", marker = "+")
        plt.plot(hessian_data3[:,0], hessian_data3[:,1], label = "Min SCHA freq 2**12", marker = "o")
        plt.plot(hessian_data4[:,0], hessian_data4[:,1], label = "Min SCHA freq 2**14", marker = ">")
#        plt.axhline(0, 0, 1, color = "k", ls = "dotted") # Draw the zero
        plt.xlabel("Temperature [K]")
        plt.ylabel("Frequency [cm-1]")
        plt.legend()
        plt.tight_layout()
        plt.savefig('Temp_Freq_SCHA1.png')
        plt.show()

        a = [hessian_data[:,1],hessian_data1[:,1],hessian_data2[:,1],hessian_data3[:,1],hessian_data4[:,1]]
        media_data = sum(a)/5
        plt.plot(hessian_data2[:,0], media_data)
        max_a = np.zeros(len(hessian_data[:,0]))
        min_a = np.zeros(len(hessian_data[:,0]))
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('Temp_Freq_SCHA2.png')
        plt.show()
...
class Hessiano_Vs_Configurations(object):
    def __init__(self,T0,fichero_ForceFields,configuraciones,sobol,sobol_scatter):
        # Load the dynamical matrix for the force field
        self.ff_dyn = CC.Phonons.Phonons(fichero_ForceFields, 3)
...
#        self.lowest_sscha_mode = []
    ...
    def ciclo_C(self,Fichero_final_matriz_dinamica,nqirr,indice):
        lowest_hessian_mode = []
        lowest_sscha_mode = []
...
#            print ('Configuracion=',Configuracion)
            ...
            self.minim.min_step_struc = 0.05
#            self.minim.min_step_dyn = 0.002
            self.minim.min_step_dyn = 0.0005
            self.minim.kong_liu_ratio = 0.5
#            self.minim.meaningful_factor = 0.000001
            #minim.root_representation = "root4"
            #minim.precond_dyn = False
...
#            self.ensemble.generate(Configuracion, sobol = self.sobol, sobol_scatter = self.sobol_scatter)
            self.ensemble.generate(Configuracion, sobol = self.sobol)
        ...
        # Save results on file
        np.savetxt("{}_hessian_vs_configuraciones.dat".format(indice), freq_data, header = "N conf; SSCHA mode [cm-1]; Free energy hessian [cm-1]")

    def dibuja(self):
        hessian_data = np.loadtxt("0_hessian_vs_configuraciones.dat")
        hessian_data1 = np.loadtxt("1_hessian_vs_configuraciones.dat")
        hessian_data2 = np.loadtxt("2_hessian_vs_configuraciones.dat")
        hessian_data3 = np.loadtxt("3_hessian_vs_configuraciones.dat")
        hessian_data4 = np.loadtxt("4_hessian_vs_configuraciones.dat")
        hessian_data5 = np.loadtxt("5_hessian_vs_configuraciones.dat")
        plt.figure(dpi = 120)
        plt.plot(hessian_data[:,0], hessian_data[:,1], label = "Min SCHA freq", marker = ">")
        ...
        # a = [hessian_data[:,2],hessian_data1[:,2],hessian_data2[:,2],hessian_data3[:,2],hessian_data4[:,2],hessian_data5[:,2]]
        # media_data = sum(a)/6
        # plt.plot(hessian_data2[:,0], media_data)
        # max_a = np.zeros(len(hessian_data[:,0]))
        # min_a = np.zeros(len(hessian_data[:,0]))
        ...
        # plt.fill_between(hessian_data2[:,0], max_a, min_a,alpha=0.2)
        plt.axhline(0, 0, 1, color = "k", ls = "dotted") # Draw the zero
        plt.axvline(x=2**6, ls = "dotted")
        plt.axvline(x=2**8, ls = "dotted")
        plt.axvline(x=2**10, ls = "dotted")
        plt.axvline(x=2**12, ls = "dotted")
        plt.axvline(x=2**14, ls = "dotted")
...
#    Temperatura_i = np.linspace(50, 300, 6)
    Temperatura_i = np.linspace(50, 300, 10)
#    Configuraciones_i = [2**4,2**5,2**6,2**7,2**8,2**9,2**10,2**11,2**12,2**13,2**14]
#    Configuraciones_i = [2**9,2**10]
#    Configuraciones_i = range(16,1024,25)#range(1024,32,-32) #range(32, 512, 2)
...
#    Fichero_dyn_SnTe = "final_sscha_T300_"
    ...
    configuraciones = [2**6,2**8,2**10,2**12,2**14]
    sobol = True
    sobol_scatter = 0.0

#    Calculo = Calculo_inicial(Fichero_ForceFields,Fichero_dyn_SnTe,nqirr,configuraciones[0],sobol,sobol_scatter)
#    Calculo.ensambla(T0)
...
#    HessianoVsTemperatura = Hessiano_Vs_Temperatura(T0,Temperatura_i,Fichero_ForceFields,sobol,sobol_scatter)
#    for indice in range(5):
#        HessianoVsTemperatura.ciclo_T(Fichero_final_matriz_dinamica,nqirr,indice,configuraciones[indice])
#    HessianoVsTemperatura.dibuja()

#    HessianoVsConfiguraciones = Hessiano_Vs_Configurations(T0,Fichero_ForceFields,Configuraciones_i,sobol,sobol_scatter)
    HessianoVsConfiguraciones = Hessiano_Vs_Configurations(T0,Fichero_ForceFields,configuraciones,sobol,sobol_scatter)
    print ('Configuraciones=',configuraciones)
    for indice in range(6):
        HessianoVsConfiguraciones.ciclo_C(Fichero_final_matriz_dinamica,nqirr,indice)
    print ('Sobol=',sobol)
