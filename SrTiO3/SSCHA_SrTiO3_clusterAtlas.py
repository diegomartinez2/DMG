...
class Hessiano_Vs_Temperatura(object):
    def __init__(self,T0,temperatura_i,configuraciones,sobol,sobol_scatter):
        ...
        command = 'srun pw.x -i PREFIX.pwi > PREFIX.pwo'
...
#            self.minim.neglect_symmetries = True    # *test*
            ...
            mi_cluster = Send_to_cluster(hostname = 'diegom@ekhi.cfm.ehu.es', label = 'SrTiO3', n_pool = 20, # n_pool must be a divisor of the k_points example 5x5x5=125 n_pool= 5 or 25
                n_cpu = 40, time = '24:00:00', mpi_cmd = 'srun' ) #test with 5 pools for QE; note reducing the n_pool reduces the memory usage in the k points calculation.
...
class Send_to_cluster(object):
    def __init__(self,hostname = 'diegom@atlas-fdr.sw.ehu.es', pwd = None,
...
module list
echo "load module"
module load QuantumESPRESSO/6.7-intel-2019b
echo "list modules"
module list
echo "calculate now"
        """
        ...
        self.cluster.binary = "pw.x -i PREFIX.pwi > PREFIX.pwo"  #<--- No need for mpirun it's set with mpi_cmd
        ...
        self.cluster.batch_size = 50

        #  Now many DFT calculation do you want to run inside each job?
        self.cluster.job_number = 1

        # Fix problem with Atlas:
        self.cluster.submit_command="source /etc/profile.d/lmod.sh; cd /scratch/diegom/; sbatch"
...
class  SrTiO3_free_energy_ab_initio(object):
    ...
    def relax(self, TEMPERATURE, N_CONFIGS, NQIRR):
        ...
        # Initialize the NVT simulation
        mi_cluster = Send_to_cluster(hostname = 'diegom@atlas-fdr.sw.ehu.es', label = 'SrTiO3_512_T50K', n_pool = 1,
           n_cpu = 2, time = '24:00:00', mpi_cmd = 'srun' ) #test with 5 pools for QE
