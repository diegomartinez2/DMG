...
#---test--^--multiprocessing*****
    ...
    def a2F_new(self,x):
        ...
        gauss_width = 100*self.from_cm1_to_eV*width #(units.invcm/units.Hartree) #0.00002 # test the units of this... should be aprox. 5 cm-1 (1, 5 or 10)
        # summa = 0
        ...
        if (method == 1):
            ...
            Nef = self.Nef
            for i in range(len(center)):
                summa += self.Lambda_q(gamma_q[i],omega_q[i],Nef)*omega_q[i] * self.gaussian(x,omega_q[i],gauss_width[i])
