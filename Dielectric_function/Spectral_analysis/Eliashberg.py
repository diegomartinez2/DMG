...
#        self.Ne_0[np.where(energy==0.0)]
    ...
    def Lambda_q(self,gamma_q,omega_q,Nef):
        ...
        Lamb_q: Lambda(q)
        """
        Lamb_q=(1/(np.pi*Nef)) * (gamma_q/omega_q**2) *self.from_GHz_to_Hartree/((self.from_cm1_to_Hartree**2) * self.from_Ry_to_Hartree)#fix from omega to omega² (*2 as Ry to Hartree for test)
        return Lamb_q
